...
# 閾値: score > 0.7 でNG判定
# リトライポリシー:
#   - NSFWを検知した場合、seedを+1して再生成を試行（最大3回）
#   - 3回目のリトライでもNG判定となった場合、生成を中断し旧画像を維持
#   - ユーザーには「生成エラー/リトライ保留中」を通知
```
...
#### 色指定 (Theme: Emerald Wash)

```yaml
背景 (Aqua Mist): #E8F7F5
グラデーション: linear-gradient(180deg, #E8F7F5 0%, #FFF5E6 100%) (透明度 60%)
サーフェス: #D9F1EE
アクセント (Emerald Core): #1DBA9D
テキスト(Primary): #1C3E3A
テキスト(Secondary): #4F7470
テキスト(Caption): #7FA7A3
危険色 (Muted Coral): #C98B8B
```

#### レイアウト・形状ルール

- **角丸**: 画像 20px / ボタン 18px / カード 16px
- **影**: `rgba(28, 62, 58, 0.06)`, blur 28
- **アニメーション**: 
  - Before/After: Before 1.4s -> Crossfade 1.1s (easeInOutSine, blur 2px -> 0px)
  - ボタン: scale 0.98, 明度 -4%, 120ms (バウンス/リップル禁止)
- **水彩にじみ**: 背景に透明度 8% のテクスチャ。進行時 14% に強化。
...
### 10.7 エッジケース運用ルール（実装詳細の明文化）

#### 10.7.1 進行判定とタイムゾーン
- 24時間判定は常にUTC基準（`last_progressed_at + 24h`）で行う。
- ユーザーのタイムゾーン変更や日付跨ぎのみで進行可にはならない。
- サマータイム変動があってもUTC基準で判定を維持する。

#### 10.7.2 リクエスト冪等性
- `/api/v1/progress` および `/api/v1/ad/reward` は冪等キー対応を前提とする。
- 同一キーの重複リクエストは二重適用せず、同一結果を返す。
- タイムアウト時は指数バックオフで最大3回再試行する。

#### 10.7.3 課金検証失敗時
- 購入検証タイムアウト時は entitlement を pending 扱いとし、バックグラウンド再検証を行う。
- 検証失敗時は entitlement を付与しない。
- 返金通知受信時は対象entitlementを決定論的に剥奪する。
...
| 2.1 | 2026-02-10 | AI Agent | Skillsメタデータ、dynamic運用ループ、エッジケース運用ルールを追記 |
| 2.2 | 2026-02-11 | AI Agent | フェーズ1決定事項（NSFW閾値/リトライ数、収益モデル、KPI）を同期 |
